...
boxplot(Count ~ Age, data = census2011)
boxplot(Count ~ Income, data = census2011)
boxplot(Count ~ Indigenous, data = census2011)
boxplot(Count ~ Indigenous.Status, data = census2011)

pairs(~State+Age+Income+Indigenous.Status+Indigenous+Sex+Count, data=census2011, main="Simple Scatterplot Matrix")

#library(lattice)
#splom(census2011[c(2,10,11,12,13,14,15,17)], groups=census2011$Indigenous.Status, data=census2011,
...
boxplot(Count ~ Age, data = census2016)
boxplot(Count ~ Income, data = census2016)
boxplot(Count ~ Indigenous, data = census2016)
boxplot(Count ~ Indigenous.Status, data = census2016)
...
# Correlation for 2011 Census data only
  ...
  #library(corrgram)
  #corrgram(censusIndigenous[which(censusIndigenous$Year == "2011"),], order=TRUE, lower.panel=panel.shade,
  ...
  nums <- sapply(censusIndigenous[which(censusIndigenous$Year == "2011"),], is.numeric)
  numeric_data <- censusIndigenous[ , nums]
...
In this section the normality of count data for the Indigenous population is examined.
...
data <- censusIndigenous[which(censusIndigenous$Year == "2011"),]$Count
...
census2011 <- censusIndigenous[which(censusIndigenous$Year == "2011"),]

# Stepwise Regression
library(MASS)
fit <- lm(Count ~ ., data=census2011)
...
attach(census2011)
leaps<-regsubsets(Count ~ .,data=census2011, nbest=10)
detach(census2011)
